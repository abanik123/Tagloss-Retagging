...
sim_function <- function(params){
  ...
  r_L <- r_R <- r_Lf <- r_Rf <- matrix(0, N, nyears)
  ...
  for (i in 1:N) {
    ...
    if (first[i] < nyears) {
      ...
      for (t in (first[i]+1):nyears) {
        ...
        #if (h[i,j-1] == 2 && j > (first[i]+1) && g[i, j-1] == 3) {
        ...
        #if (h[i,j-1] == 2 && j > (first[i]+1) && g[i, j-1] == 2) {
  ...
  # Need r_Lf and r_Rf for getting s_indi_L and s_indi_R
  ...
  r_Lf <- as.matrix(convert_first1_to_ones(r_L))
  r_Rf <- as.matrix(convert_first1_to_ones(r_R))
  ...
  retag_L <- retag_L[, -ncol(retag_L)]
  retag_R <- retag_R[, -ncol(retag_R)]
  ...
  retag_L    <- retag_L[keep, , drop = FALSE]
  retag_R    <- retag_R[keep, , drop = FALSE]
  ...
  g_inits <- tag
  g_inits[is.na(tag)] <- 1
  #g_inits[is.na(tag)] <- sample(1:3, sum(is.na(tag)), replace = TRUE)
  ...
  sim_lam_L <- MCMCsummary(samples_sim, params = "lam_L", round = 5)$mean
  sim_lam_R <- MCMCsummary(samples_sim, params = "lam_R", round = 5)$mean
  sim_phi <- MCMCsummary(samples_sim, params = "phi", round = 5)$mean
  sim_p <- MCMCsummary(samples_sim, params = "p", round = 5)$mean
  
  d_b = data.frame(sim_lam_L, sim_lam_R, sim_phi, sim_p)
  
  write.csv(d_b, paste0("lam_L", sim_lam_L, "_lam_R",sim_lam_R,
...
params= list(lam_L = 0.8, lam_R = 0.8,
             phi = 0.7, p = 0.4,
             iter=iter)
sim_function(params)