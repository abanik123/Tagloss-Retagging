...
code_m1 <- nimbleCode({
  ...
  for (i in 1:N) {
    for (t in seq(f_1[i] + 1, nyears, 
      ...
      pt[4,1,i,t] <- ((s_L[i,t-1]*lam_Lf)+((1-s_L[i,t-1])*r_Lf[i,t-1]*r_Ln[i,t-1]*lam_Ln))*
                    ((s_R[i,t-1]*lam_Rf)+((1-s_R[i,t-1])*r_Rf[i,t-1]*r_Rn[i,t-1]*lam_Rn))
      
      pt[4,2,i,t] <- ((s_L[i,t-1]*lam_Lf)+((1-s_L[i,t-1])*r_Lf[i,t-1]*r_Ln[i,t-1]*lam_Ln))*
                    (1-((s_R[i,t-1]*lam_Rf)+((1-s_R[i,t-1])*r_Rf[i,t-1]*r_Rn[i,t-1]*lam_Rn)))
      
      pt[4,3,i,t] <- (1-((s_L[i,t-1]*lam_Lf)+((1-s_L[i,t-1])*r_Lf[i,t-1]*r_Ln[i,t-1]*lam_Ln)))*
                    ((s_R[i,t-1]*lam_Rf)+((1-s_R[i,t-1])*r_Rf[i,t-1]*r_Rn[i,t-1]*lam_Rn))
      
      pt[4,4,i,t] <- (1-((s_L[i,t-1]*lam_Lf)+((1-s_L[i,t-1])*r_Lf[i,t-1]*r_Ln[i,t-1]*lam_Ln)))*
                    (1-((s_R[i,t-1]*lam_Rf)+((1-s_R[i,t-1])*r_Rf[i,t-1]*r_Rn[i,t-1]*lam_Rn)))
    }
  }
...
pdf(file = "ms_wpt_mp_lam2.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "ms_wpt_lam2", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "ms_wpt_lam2_sum.csv")